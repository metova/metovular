...
angular.module("templates", []).run(["$templateCache", function($templateCache) {$templateCache.put("formFor/form-for-tpl.html","<form id=\"{{ getItem().type.toLowerCase() }}-form\" ng-submit=\"submit()\" ng-transclude=\"\" class=\"{{ getFormClass() }}\"></form><div ng-if=\"getOption(\'showSubmit\') || getOption(\'showCancel\')\"><button ng-click=\"submit()\" ng-if=\"getOption(\'showSubmit\')\" class=\"btn btn-primary pull-right\">{{ getSubmitValue() }}</button><button ng-click=\"cancel($event)\" ng-if=\"getOption(\'showCancel\')\" class=\"btn btn-default\">Cancel</button><div class=\"clearfix\"></div></div>");
$templateCache.put("formFor/input-tpl.html","<div ng-class=\"{ \'has-error\': getItem().errors[getParam(\'for\')] }\" ng-if=\"getParam(\'if\')\" class=\"form-group\"><label for=\"{{ getParam(\'for\') }}\" class=\"control-label {{ getOption(\'labelClass\') }}\">{{ getParam(\'label\') }}</label><div ng-switch=\"getParam(\'type\')\" class=\"{{ getOption(\'controlClass\') }}\"><input id=\"{{ getParam(\'id\') }}\" type=\"{{ getParam(\'type\') }}\" maxlength=\"{{ getParam(\'maxlength\') }}\" ng-required=\"getParam(\'required\') === \'true\' ? \'true\' : \'\'\" ng-model=\"getItem()[getParam(\'for\')]\" placeholder=\"{{ getParam(\'placeholder\') }}\" ng-switch-default=\"\" ng-disabled=\"getParam(\'disabled\') === \'true\' ? true : \'\'\" class=\"form-control\"/><input id=\"{{ getParam(\'id\') }}\" type=\"text\" maxlength=\"10\" ng-required=\"getParam(\'required\') === \'true\' ? \'true\' : \'\'\" ng-model=\"getItem()[getParam(\'for\')]\" placeholder=\"{{ getParam(\'placeholder\') }}\" ng-datepicker=\"\" ng-switch-when=\"datepicker\" ng-disabled=\"getParam(\'disabled\') === \'true\' ? true : \'\'\" class=\"form-control\"/><div id=\"{{ getParam(\'for\') }}\" ng-switch-when=\"radio-buttons\" class=\"btn-group btn-group-justified\"><label ng-model=\"getItem()[getParam(\'for\')]\" btn-radio=\"button.value\" ng-repeat=\"button in getParam(\'_options\') track by $index\" class=\"btn btn-default\">{{ button.label }}</label></div><div id=\"{{ getParam(\'id\') }}\" ng-switch-when=\"platform-check-buttons\" class=\"btn-group\"><label ng-model=\"getItem()[getParam(\'for\')][button.value]\" btn-radio=\"button.value\" btn-checkbox=\"btn-checkbox\" ng-repeat=\"button in getParam(\'_options\') track by $index\" class=\"btn btn-default\"><platform-icon data=\"button.label\"></platform-icon></label></div><div ng-switch-when=\"select2\"><ui-select id=\"{{ getParam(\'id\') }}\" name=\"{{ getParam(\'for\') }}\" ng-model=\"getItem()[getParam(\'for\')]\" theme=\"select2\" ng-required=\"getParam(\'required\') === \'true\' ? \'true\' : \'\'\" ng-disabled=\"getParam(\'disabled\') === \'true\' ? true : \'\'\" class=\"form-control\"><ui-select-match placeholder=\"Select from the list...\">{{ $select.selected.label }}</ui-select-match><ui-select-choices repeat=\"option.value as option in getParam(\'_options\') | filter:$select.search\"><div ng-bind-html=\"option.label | highlight: $select.search\"></div></ui-select-choices></ui-select></div><div ng-switch-when=\"select2-multi\"><ui-select id=\"{{ getParam(\'id\') }}\" name=\"{{ getParam(\'for\') }}\" ng-model=\"getItem()[getParam(\'for\')]\" theme=\"select2\" ng-required=\"getParam(\'required\') === \'true\' ? \'true\' : \'\'\" multiple=\"multiple\" ng-disabled=\"getParam(\'disabled\') === \'true\' ? true : \'\'\" class=\"form-control\"><ui-select-match placeholder=\"Select from the list...\">{{ $item.label }}</ui-select-match><ui-select-choices repeat=\"option.value as option in getParam(\'_options\') | filter:$select.search\"><div ng-bind-html=\"option.label | highlight: $select.search\"></div></ui-select-choices></ui-select></div><select id=\"{{ getParam(\'id\') }}\" name=\"{{ getParam(\'for\') }}\" ng-model=\"getItem()[getParam(\'for\')]\" ng-required=\"getParam(\'required\') === \'true\' ? \'true\' : \'\'\" ng-options=\"option.value as option.label for option in getParam(\'_options\')\" ng-switch-when=\"select\" ng-disabled=\"getParam(\'disabled\') === \'true\' ? true : \'\'\" class=\"form-control\"></select><div><span ng-repeat=\"error in getItem().errors[getParam(\'for\')]\" class=\"help-block\">{{ error }}</span></div></div></div>");}]);
(function() {
  var app;
